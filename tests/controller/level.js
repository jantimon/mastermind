...
define([
  ...
  function (Backbone, _, $, levelController, GuessModel) {
    ...
    function levelInitializationHelper(callback, levelIndex, solution) {
      ...
      levelController.execute.call(new Backbone.Router(), level, index);
      var $domElement = $('.decoding-board:last');
      callback(level, $domElement, index);
      $domElement.remove();
    }
    ...
    describe('Level controller', function () {
      ...
      it('should add a dom element', function () {
        levelInitializationHelper(function () {
          // Overwrite secret combination
          expect($('.decoding-board:last').length).toEqual(1);
        });
      });

      it('should start with empty rows', function () {
        levelInitializationHelper(function (level) {
      ...
      it('should hide the secret combination during the game', function () {
        levelInitializationHelper(function (level, $rootDomElement) {
          expect($rootDomElement.find('.solution.revealed').length).toBeFalsy();
        });
      });


      it('should set the row to confirmed after the state was set to complete', function () {
        levelInitializationHelper(function (level) {
      ...
      it('should not end the game after receiving a wrong guess', function () {
        levelInitializationHelper(function (level) {
          var row = level.getCurrentGuessRow();
          row.get('pegs').each(function (peg) {
            peg.set({
      ...
      it('should enable the next row after receiving a wrong guess', function () {
        levelInitializationHelper(function (level) {
      ...
      it('should give correct hints', function () {
        levelInitializationHelper(function (level) {
          var row = level.getCurrentGuessRow();
      ...
      it('should end after receiving only wrong guesses', function () {
        levelInitializationHelper(function (level) {
      ...
      it('should know that the player lost  after receiving only wrong guesses', function () {
        levelInitializationHelper(function (level) {
          level.get('guessRows').each(function () {
            var row = level.getCurrentGuessRow();
            if (row.isChangeable()) {
      ...
      it('should end the game after receiving a correct guess', function () {
        levelInitializationHelper(function (level) {
          var row = level.getCurrentGuessRow();
          row.get('pegs').at(0).set('color', 'black');
          row.get('pegs').at(1).set('color', 'white');
      ...
      it('should know that the player won after receiving a correct guess', function () {
        levelInitializationHelper(function (level) {
          var row = level.getCurrentGuessRow();
          row.get('pegs').at(0).set('color', 'black');
          row.get('pegs').at(1).set('color', 'white');
      ...
      it('should reveal the secret combination after the game ended', function () {
        levelInitializationHelper(function (level, $rootDomElement) {
          var row = level.getCurrentGuessRow();
          row.get('pegs').at(0).set('color', 'black');
          row.get('pegs').at(1).set('color', 'white');
